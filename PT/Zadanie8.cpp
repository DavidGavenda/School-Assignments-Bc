...
double average(const initializer_list<int>& data) {
	...
	double value = accumulate(data.begin(), data.end(), 0);
	value = value / (data.size());
	return value; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
void helpAfterExam2(list<int>& points) noexcept {
	for (int& a : points) a = incrementPointsIfItHelps(a);
}
