...
queue<int> reverse(const list<int>& data) noexcept {
	queue<int> opacne;
	reverse_iterator<list<int>::const_iterator> rit = data.rbegin();
	for (;rit != data.rend();rit++) {
		opacne.push(*rit);
	}
	return opacne; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
vector<vector<int>> create2dVector(const size_t size1, const size_t size2, const int value) noexcept {
	vector<vector<int>> vektor(size1, vector<int>(size2));
	for (int i = 0;i < size1;i++) {
		for (int j = 0; j < size2;j++) {
			vektor[i][j] = value;
		}
	}
	return vektor; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
list<string> translate(const map<string, string>& translator, const list<string>& sentence) noexcept {
	...
	list<string> listok;
	for (list<string>::const_iterator i = sentence.begin(); i != sentence.end(); i++) {
		if (translator.find(*i) != translator.end()) {
			listok.push_back(translator.find(*i)->second);
		}
		else {
			listok.push_back("?");
		}
	}
	return listok; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
bool areParenthesisPaired(const string& mathExpression) noexcept {
	...
	for (int i = 0; i < mathExpression.size(); i++) {
		if ((mathExpression[i] == '(') ||
			...
			(mathExpression[i] == '{') ||
			(mathExpression[i] == '<')) {
			panStack.push(mathExpression[i]);
		}
		else if (!panStack.empty()) {
			if ((panStack.top()) == '(' && mathExpression[i] == ')' ||
				(panStack.top()) == '[' && mathExpression[i] == ']' ||
				(panStack.top()) == '{' && mathExpression[i] == '}' ||
				(panStack.top()) == '<' && mathExpression[i] == '>') {
				panStack.pop();
			}
...
list<int> createList(const list<int>::size_type length, const list<int>::size_type indexOfOne) {
	...
	try {
		...
		for (int i = 0;i < length;i++) {
			if (i == indexOfOne) vratnik.push_back(1);
			else vratnik.push_back(0);
		}
		return vratnik;
	}
