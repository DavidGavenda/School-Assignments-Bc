...
public:
	...
	char getChar(const size_t index) const {
		if (strlen(data) == 0 || index >= strlen(data)) {
			return 0;
		}
	...
	void append(const char* text) {
		char* novy = new char[strlen(data) + strlen(text)];
		strcpy(novy, data);
		novy = strcat(novy, text);
		delete[] data;
		data = new char[strlen(novy) + 1];
		strcpy(data, novy);
		delete[] novy;
	}
...
void basicTestString() {
	...
	// d)
	//size_t length1 = str2.getLength();
	//assert(length1 == 0);
	//size_t length2 = str2.getLength();
	//assert(length2 == 11);

	// e)
	//char letter1 = str1.getChar(5);
	//assert(letter1 == '\0');
